...
const ActionProgressBar = ({ action }: { action?: GameAction }) => {
	if (!action) return <div className="bg-neutral-4 relative h-1 w-full"></div>;
	const now = Date.now();
	const durationMs = action.timesOutAt - now;

	// return seconds remaining
	return (
		<div className="bg-neutral-4 relative h-1 w-full">
			<div
				style={{ animationDuration: `${durationMs}ms` }}
				className="animate-to-zero-width bg-neutral-9 absolute h-full rounded-full"
			/>
		</div>
	);
};
...
export const PlayerSide = ({
	...
	return (
		<div className={clsx('flex flex-col items-center gap-2', { 'flex-col-reverse': relative === 'opponent' })}>
			...
			<Badge id={side + '-hp'} className="flex items-center gap-2">
